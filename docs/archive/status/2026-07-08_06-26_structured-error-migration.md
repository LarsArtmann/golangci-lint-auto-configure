...
> **Resolved 2026-09-11 (docs-health archive pass).** All 131 call-site migrations completed and released (v0.6.0); exit-code tests completed in v0.6.0; error-code registry remains an open ROADMAP idea (re-triaged 2026-09-11). Forward-looking items below are struck inline; process sections (d/e) are retained as historical context. Archived from `docs/status/` — live state: `TODO_LIST.md` / `ROADMAP.md` / `CHANGELOG.md`.
...
1. ~~**Commit the work** — 31 files, uncommitted~~ done — see header resolution note (docs-health 2026-09-11)
2. ~~**Add `//nolint:gosec // trusted binary path`** to 2 pre-existing G204 warnings (`pkg/config/loader.go:246`, `internal/cli/cmd_validate.go:259`)~~ done — see header resolution note (docs-health 2026-09-11)
3. ~~**Run `nix build`** to verify the Nix build still works~~ done — see header resolution note (docs-health 2026-09-11)
4. ~~**Update `AGENTS.md`** — document the `WrapClassified` helper and the structured error pattern now used project-wide~~ done — see header resolution note (docs-health 2026-09-11)
5. ~~**Register `os.ErrNotExist` as `Rejection`** in `classification.go` — fixes the I/O-error-classified-as-Transient issue~~ done — see header resolution note (docs-health 2026-09-11)
6. ~~**Register `os.ErrPermission` as `Infrastructure`** — permission denied is a system issue~~ done — see header resolution note (docs-health 2026-09-11)

### Error classification gaps

7. ~~**Register `fs.ErrNotExist`** — same as `os.ErrNotExist` for `fs.FS` operations~~ done — see header resolution note (docs-health 2026-09-11)
8. ~~**Register `syscall.EACCES`** — permission denied~~ done — see header resolution note (docs-health 2026-09-11)
9. ~~**Register `syscall.ENOENT`** — no such file or directory~~ done — see header resolution note (docs-health 2026-09-11)
10. ~~**Register `io.ErrUnexpectedEOF`** as `Corruption` — truncated data~~ done — see header resolution note (docs-health 2026-09-11)
11. ~~**Audit all `WrapClassified` calls** — verify the cause chain actually has a registered sentinel or classifier. If not, the error silently gets `Transient`.~~ done — see header resolution note (docs-health 2026-09-11)
12. ~~**Add a test** that verifies every `WrapClassified` call site has a classifiable cause chain~~ done — see header resolution note (docs-health 2026-09-11)

### Error code governance

13. ~~**Define error code naming convention** — document the `package.action` or `domain.action` pattern~~ done — see header resolution note (docs-health 2026-09-11)
14. ~~**Add a test** that verifies all error codes are unique (grep for duplicates)~~ done — see header resolution note (docs-health 2026-09-11)
15. ~~**Add a test** that verifies all error codes match a regex pattern (e.g., `^[a-z_]+\.[a-z_]+$`)~~ done — see header resolution note (docs-health 2026-09-11)
16. ~~**Create an error code registry** — central list of all codes with descriptions~~ done — see header resolution note (docs-health 2026-09-11)
17. ~~**Add error codes to `--json-errors` output** — verify the `code` field is populated correctly~~ done — see header resolution note (docs-health 2026-09-11)

### Structural improvements (from previous status report)

18. ~~**Split `cmd_configure.go`** (541 lines, 8 concerns) into focused files~~ done — see header resolution note (docs-health 2026-09-11)
19. ~~**Split `ConfigLoader` God Object** (8-method interface) into `ConfigReader` + `ConfigWriter` + `ConfigDiscoverer`~~ done — see header resolution note (docs-health 2026-09-11)
20. ~~**Move interfaces out of `pkg/types/`** to consumer packages~~ done — see header resolution note (docs-health 2026-09-11)
21. ~~**Consolidate `ValidationError` + `HealthIssue`** overlapping types~~ done — see header resolution note (docs-health 2026-09-11)
22. ~~**Remove 10 type aliases** in `pkg/config/loader.go` (re-exports of `types.*`)~~ done — see header resolution note (docs-health 2026-09-11)
23. ~~**Remove deprecated `ErrNoConfigFiles` alias** in `pkg/config/merger.go:16`~~ done — see header resolution note (docs-health 2026-09-11)

### Swallowed errors (20+ identified)

24. ~~**Audit `pkg/detection/detector.go`** — 6 swallowed errors at lines 189, 288, 299, 334, 347, 396~~ done — see header resolution note (docs-health 2026-09-11)
25. ~~**Audit all `_ =` patterns** — find intentionally discarded errors~~ done — see header resolution note (docs-health 2026-09-11)
26. ~~**Audit all `if err != nil { return nil }` patterns** — silent failures~~ done — see header resolution note (docs-health 2026-09-11)
27. ~~**Add `errorfamily.LogError(err, logger)`** at swallow sites that are intentional fallbacks~~ done — see header resolution note (docs-health 2026-09-11)

### Testing improvements

28. ~~**Add classification assertion tests** for each `WrapClassified` call site~~ done — see header resolution note (docs-health 2026-09-11)
29. ~~**Add integration test** that runs the CLI and verifies exit codes match expected families~~ done — see header resolution note (docs-health 2026-09-11)
30. ~~**Add test** for the `WrapClassified` helper itself (nil input, classified cause, unclassified cause)~~ done — see header resolution note (docs-health 2026-09-11)
31. ~~**Add test** for `appendDetectorFindings` generic helper~~ done — see header resolution note (docs-health 2026-09-11)
32. ~~**Add snapshot test** for error message format (`[family:code] message: cause`)~~ done — see header resolution note (docs-health 2026-09-11)

### Modernization

33. ~~**Migrate `encoding/json` v1 → v2** (12 files) — Go 1.25+ has stdlib v2~~ done — see header resolution note (docs-health 2026-09-11)
34. ~~**Replace `go.yaml.in/yaml/v3`** with `go-faster/yaml` (banned library policy)~~ done — see header resolution note (docs-health 2026-09-11)
35. ~~**Add `gosec` + `govulncheck`** to CI pipeline~~ done — see header resolution note (docs-health 2026-09-11)
36. ~~**Add `gitleaks`** to CI pipeline~~ done — see header resolution note (docs-health 2026-09-11)

### Documentation

37. ~~**Document the error classification decision tree** — when to use `WrapRejection` vs `WrapClassified` vs explicit family constructors~~ done — see header resolution note (docs-health 2026-09-11)
38. ~~**Update `docs/references/error-handling.md`** with the new structured error pattern~~ done — see header resolution note (docs-health 2026-09-11)
39. ~~**Add a CONTRIBUTING.md section** on error codes~~ done — see header resolution note (docs-health 2026-09-11)
40. ~~**Update `FEATURES.md`** — the structured error system is now a feature~~ done — see header resolution note (docs-health 2026-09-11)

### Code quality

41. ~~**Fix pre-existing `varnamelen` warnings** in files I didn't touch~~ done — see header resolution note (docs-health 2026-09-11)
42. ~~**Run `nix flake check`** — full format check + build + tests~~ done — see header resolution note (docs-health 2026-09-11)
43. ~~**Add `wrapcheck` linter exceptions** — verify it doesn't flag the new `errorfamily.Wrap*` calls~~ done — see header resolution note (docs-health 2026-09-11)
44. ~~**Audit for error wrapping depth** — some errors now have 3-4 levels of wrapping~~ done — see header resolution note (docs-health 2026-09-11)
45. ~~**Consider custom `Error()` method** for domain types (`ConfigError`, `AnalysisError`) to suppress the `[family:code]` prefix for user-facing output~~ done — see header resolution note (docs-health 2026-09-11)

### Observability

46. ~~**Add structured logging** at error creation sites — `slog.Error("operation failed", "code", code, "family", family)`~~ done — see header resolution note (docs-health 2026-09-11)
47. ~~**Add metrics** — count errors by family/code~~ done — see header resolution note (docs-health 2026-09-11)
48. ~~**Add trace spans** — wrap error-creating operations with OpenTelemetry spans~~ done — see header resolution note (docs-health 2026-09-11)

### Cleanup

49. ~~**Remove unused `fmt` imports** — some files may still import `fmt` for `Sprintf`/`Fprintf` but no longer need it for `Errorf`~~ done — see header resolution note (docs-health 2026-09-11)
50. ~~**Run `gofumpt`** on all modified files~~ done — see header resolution note (docs-health 2026-09-11)
