...
> **Resolved 2026-09-11 (docs-health archive pass).** showDiff parameterized (ADR-005); --diff/--check tests, CLI unit tests, merger fuzz tests, and `cmd/coverage-check` shipped through v0.6.0; gogenfilter scanner coverage consciously deferred twice (ROADMAP); branded configPath type never prioritized. Forward-looking items below are struck inline; process sections (d/e) are retained as historical context. Archived from `docs/status/` — live state: `TODO_LIST.md` / `ROADMAP.md` / `CHANGELOG.md`.
...
| -- | ------------------------------------------------------------------------------ | ------ | ------ | ------------------------------------------------------------------------------------------------------------------ |
| 1  | ~~**Fix showDiff package variable → parameter**~~ done — see header resolution note (docs-health 2026-09-11) | High   | 15min  | Removes hidden dependency, enables unit testing                                                                    |
| 2  | ~~**Write fuzz test for pkg/config/merger.go** (not types.Set)~~ done — see header resolution note (docs-health 2026-09-11) | High   | 30min  | Merger is 63.4% coverage, complex merge logic                                                                      |
| 3  | ~~**Unit tests for cmd_configure.go internal functions**~~ done — see header resolution note (docs-health 2026-09-11) | High   | 60min  | CLI coverage 9.1% → target 30%. Test `runFixerMode`, `finalizeFixerResult`, `effectiveDryRunForCheckDiff` directly |
| 4  | ~~**Fix nixfmt-standalone in devShell** so pre-commit passes~~ done — see header resolution note (docs-health 2026-09-11) | High   | 30min  | Root cause of all --no-verify bypasses                                                                             |
| 5  | ~~**Add --diff integration tests** (additions, removals, dry-run)~~ done — see header resolution note (docs-health 2026-09-11) | Medium | 30min  | User-facing output untested                                                                                        |
| 6  | ~~**Add --check mode tests** (optimal config, dry-run combo)~~ done — see header resolution note (docs-health 2026-09-11) | Medium | 30min  | Only 1 of 4 planned tests done                                                                                     |
| 7  | ~~**Branded type for configPath** (prevent empty/dir paths)~~ done — see header resolution note (docs-health 2026-09-11) | Medium | 45min  | Type safety improvement                                                                                            |
| 8  | ~~**gogenfilter coverage 63.9% → 80%**~~ done — see header resolution note (docs-health 2026-09-11) | Medium | 45min  | Untouched this session                                                                                             |
| 9  | ~~**Consider Result type for CLI commands**~~ done — see header resolution note (docs-health 2026-09-11) | Medium | 60min  | Enables assertion-based testing without binary exec                                                                |
| 10 | ~~**--json-errors: reconsider PascalCase vs snake_case**~~ done — see header resolution note (docs-health 2026-09-11) | Low    | 15min  | SARIF uses snake_case; we should match the ecosystem                                                               |
| 11 | ~~**Convert coverage-check.sh to a Go test**~~ done — see header resolution note (docs-health 2026-09-11) | Low    | 30min  | More portable, testable                                                                                            |
| 12 | ~~**HandleError at CLI boundary**~~ done — see header resolution note (docs-health 2026-09-11) | Low    | 45min  | Current slog.Error works but isn't structured                                                                      |
| 13 | ~~**Rewrite Pareto plan as HTML per skill spec**~~ done — see header resolution note (docs-health 2026-09-11) | Low    | 30min  | Compliance with skill format                                                                                       |
| 14 | ~~**AGENTS.md gotcha #4 cleanup**~~ done — see header resolution note (docs-health 2026-09-11) | Low    | 10min  | Still references old "known issue" context                                                                         |
| 15 | ~~**Consider using go-error-family's Error.JSON()** for --json-errors~~ done — see header resolution note (docs-health 2026-09-11) | Low    | 20min  | Instead of hand-rolled jsonError struct                                                                            |
| 16 | ~~**Integration test: SARIF output validates against schema**~~ done — see header resolution note (docs-health 2026-09-11) | Low    | 30min  | CI consumers depend on valid SARIF                                                                                 |
| 17 | ~~**Consolidate configPosition helper** into go-finding upstream~~ done — see header resolution note (docs-health 2026-09-11) | Low    | 60min  | The `Line: 1` default for config-level findings is a reusable pattern                                              |
| 18 | ~~**Document the showDiff → parameter migration as an ADR**~~ done — see header resolution note (docs-health 2026-09-11) | Low    | 15min  | Records the decision for future contributors                                                                       |
| 19 | ~~**Add testifylint enable-all check to CI** (banned in BuildFlow)~~ done — see header resolution note (docs-health 2026-09-11) | Low    | 15min  | Consistency across ecosystem                                                                                       |
| 20 | ~~**Benchmark the config merger** (regression detection)~~ done — see header resolution note (docs-health 2026-09-11) | Low    | 30min  | Merger runs on every multi-config project                                                                          |
| 21 | ~~**Consider koanf for config loading** (BuildFlow + hierarchical-errors use it)~~ done — see header resolution note (docs-health 2026-09-11) | Low    | 120min | Replaces hand-rolled YAML/TOML/JSON dispatch                                                                       |
| 22 | ~~**Property test: fixer is idempotent** (fix twice = fix once)~~ done — see header resolution note (docs-health 2026-09-11) | Low    | 30min  | Core correctness invariant                                                                                         |
| 23 | ~~**Snapshot test for HTML report output**~~ done — see header resolution note (docs-health 2026-09-11) | Low    | 45min  | Templ reports change silently                                                                                      |
| 24 | ~~**Add `--quiet` flag for CI** (errors-only output)~~ done — see header resolution note (docs-health 2026-09-11) | Low    | 30min  | Reduces noise in GitHub Actions                                                                                    |
| 25 | ~~**Investigate making Config immutable** (all mutations via methods)~~ done — see header resolution note (docs-health 2026-09-11) | Low    | 180min | Biggest type-safety improvement but largest effort                                                                 |
