...
1-4 (above)
5. **YAML indentation preservation** (3-4h) — the #1 user-facing pain. The tool reformats 2-space→4-space aggressively, producing massive whitespace-only diffs that obscure real changes. Affects every single user.
6. **`RuleKey()` merge strategy** (2-3h) — new linter defaults don't reach 88 existing configs. The dedup key is `Path|Text|Source`, so machine-generated configs are stuck on old exclusion lists.
7. **`--force-settings` flag** (2h) — idempotency trap: the tool's own `.golangci.yml` can never be cleaned up by re-running the tool because existing settings are never overwritten.
8. **Extract `CommandContext` struct** (2-3h) — 9 package-level variables in `commands.go` act as implicit context. This was the architecture review's #1 concern and enables parallel test execution.
