...
**My question:** Is the recommendation to *replace* the existing custom error types with go-error-family constructors, or to *add* go-error-family alongside them? The existing types have domain-specific semantics (Config vs Analysis vs Report) that don't map cleanly to Rejection vs Transient. Replacing them would lose context about *where* the error occurred, while adding both creates two parallel error classification systems.
