...
> **RESOLVED (2026-07-26 followup pass): KEEP (option c), now with correct singular/plural output.** The script carries `//go:build ignore`, so it never runs in CI; the Ginkgo specs are the authoritative, always-run gate. The script remains a useful human-readable debug aid, and drift is low-impact (a drifted manual script only affects whoever runs it by hand). Singular/plural grammar fixed across all 8 checks. If drift ever bites, extract a shared `ValidateTiers()` (item 16) — deferred as YAGNI.
...
> **RESOLVED (2026-07-26 followup pass): KEEP `gochecknoglobals` in `PragmaticNoise`.** The tiers model different situations: `exhaustruct` is _unusable out of the box_ (needs per-project struct allow-lists that can't be auto-generated), justifying "never recommend." `gochecknoglobals` works fine without config but is opinionated about global state — exactly what `--pragmatic` opts out of. The distinction is now documented in the `PragmaticNoiseLinters` doc comment in `rules.go`. Promoting it changes default coverage for ~160 projects and still needs explicit user sign-off; reversible if friction data flips.
...
> **RESOLVED (2026-07-26 followup pass): DO NOT strip exhaustruct from existing configs.** This directly contradicts requirement #2 ("if somebody adds it, it should not be removed"). `NeverAutoEnable` semantics are "don't add going forward, don't remove what's there" — stripping would break the preservation guarantee. The tool also cannot reliably distinguish its own past output from a deliberate user choice, so stripping is unsafe. Existing configs keep exhaustruct (with its curated stdlib excludes); new configs never get it auto-added.
