...
Generated-code exclusion derivation is delegated to the gogenfilter
library: one source of truth, scoped derivation, and an end to the
exclusion-whack-a-mole (`.golangci.yml` exclusion rules growing on every
FixConfig run).

### Changed — Generated-code exclusions (library delegation)

- The scanner in `pkg/gogenfilter` is now a thin delegation to
  `github.com/LarsArtmann/gogenfilter/v3` `ScanProject` (v3.6.0): preset
  tables, oapi/dir heuristics, and directory-walk logic are removed from
  this repo; future library improvements flow through without
  re-implementing them here
- Scoped exclusion derivation (gogenfilter v3.6.0): a directory is
  excluded by an anchored `^dir/` pattern only when EVERY .go file under
  it (recursively) is detected-generated; otherwise each file gets a
  precise `^path$` pattern. Mixed directories no longer receive a dir
  exclusion that silently silences hand-written code
- sqlc output-dir discovery via golangci config was dropped in the
  delegation (narrower and safer: only files detected as generated by
  their content markers are excluded)

### Changed — Behavior

- Files carrying only the generic "Code generated … DO NOT EDIT" marker
  (unknown generator) now receive precise per-file exclusions;
  previously they received none and stayed linted. Accepted as the
  correct reading of the Go generated-code marker convention; consumers
  who want such files linted can remove the exclusion in config
- Preset exclusion patterns are emitted only when they match ALL
  detected files of that generator (no silent no-op patterns)

### Added

- Idempotence regression test: running FixConfig twice on a fixture tree
  adds zero duplicate exclusion rules and leaves the exclusion path set
  byte-identical
- Constants pins: `\.gen\.go$`, `_templ\.go$`, and `vendor/` in the
  default linter/formatter exclusion paths (legacy oapi-codegen layout
  coverage)

### Dependencies

- `github.com/LarsArtmann/gogenfilter/v3` v3.5.0 → v3.6.0

## [0.7.1] - 2026-09-10

### Added

- `mockery` (`mock_*.go`) and `counterfeiter` (`fake_*.go`) generator
  exclusion presets

## [0.7.0] - 2026-09-10

Shipped without a changelog section at tag time (convention gap fixed
as of 0.8.0); entries below describe the 0.7.0 tree.
